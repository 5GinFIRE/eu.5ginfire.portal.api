...
public class OSM3Client {
	...
	private static Map<Integer, OSM3Client> instances = new HashMap<>();

	public static OSM3Client getInstance(MANOprovider mano) {
		OSM3Client osmapi = instances.get(mano.getId());
		if (osmapi == null) {
			osmapi = new OSM3Client( mano );
			osmapi.init();
